...
- 下载切换：下载进行中时按钮切换为“暂停下载”，点击后可中断当前抓取并恢复初始按钮状态
- 下载提速：支持浏览器会优先走分片流式下载，移动端与不支持浏览器自动回退为原生下载兜底
...
- 下载进行中按钮会切换为“暂停下载”，点击后会中断当前下载请求
- 支持 `showSaveFilePicker` 的桌面浏览器会优先走分片流式下载，移动端默认回退到原生下载
...
- 下载期间按钮会切换为“暂停下载”，允许用户主动中断当前下载过程
- 支持的桌面浏览器优先使用分片流式下载，提升开始下载速度，并对移动端保留原生下载兜底
